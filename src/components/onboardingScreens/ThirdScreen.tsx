...
import { Dimensions, Image, StyleSheet, Text, TextInput, View, Easing, TouchableOpacity } from "react-native";
import { Circle, G, Svg } from "react-native-svg";
import { faCircleArrowRight, faCircleNotch } from "@fortawesome/free-solid-svg-icons";
import { FontAwesomeIcon } from "@fortawesome/react-native-fontawesome";
import { MotiView } from "moti";
import Animated, { runOnJS, useAnimatedProps, useAnimatedStyle, useSharedValue, withRepeat, withSequence, withTiming } from "react-native-reanimated";
...
const ThirdScreen = ({ navigation, onClick }: any): JSX.Element => {
  ...
  const StartAnimation = () => {
    ...
    progressValue.value = withRepeat(withSequence(withTiming(1.5, { duration: 900 }), withTiming(2.5, { duration: 3000 })), -1, true);
  ...
  return (
    <View style={styles.background}>
      <Image source={require("../../../resources/thirdImage.jpeg")} style={styles.backgroundImage} resizeMethod="resize" resizeMode="cover" />
              ...
              <Animated.View style={animatedStyle}>
                <Svg width={diameter} height={diameter} viewBox={`0 0 ${diameter} ${diameter}`}>
                  <G origin={`${halfCircle}, ${halfCircle}`} rotation={"-90"}>
                    ...
                    />
                    <Circle fill={"transparent"} stroke={"black"} r={radius} cx={"50%"} cy={"50%"} strokeWidth={strokeWidth} strokeOpacity={0.3} />
                  </G>
                </Svg>
              </Animated.View>
            </View>
            <TouchableOpacity onPress={() => navigation.navigate("LoginScreen")}>
              <FontAwesomeIcon style={styles.nextPageArrow} icon={faCircleArrowRight} size={40} />
            </TouchableOpacity>
          </View>
        </View>
      </View>
